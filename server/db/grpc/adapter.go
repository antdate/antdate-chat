...
const (
	...
	userDSN      = "root:antdate@tcp(39.100.125.190:33060)/antdate?parseTime=true&collation=utf8mb4_unicode_ci"
	userDatabase = "antdate"
...
func (a *adapter) Open(jsonconfig json.RawMessage) error {
	...
	a.userDB, err = sqlx.Open("mysql", userDSN)
	if err != nil {
		return err
	}
...
func (a *adapter) GetDbVersion() (int, error) {
	...
	var vers int
	err := a.db.Get(&vers, "SELECT `value` FROM chat_kvmeta WHERE `key`='version'")
	if err != nil {
...
func (a *adapter) updateDbVersion(v int) error {
	a.version = -1
	if _, err := a.db.Exec("UPDATE chat_kvmeta SET `value`=? WHERE `key`='version'", v); err != nil {
		return err
	}
	return nil
}
...
func (a *adapter) UpgradeDb() error {
	...
	if a.version == 107 {
		...
		if _, err := a.db.Exec(`UPDATE chat_users SET access=JSON_REPLACE(access, '$.Auth', 'JRWPAS') 
	...
	if a.version == 110 {
		// Users
		if _, err := a.db.Exec("ALTER TABLE chat_users MODIFY state SMALLINT NOT NULL DEFAULT 0 AFTER updatedat"); err != nil {
			return err
		}

		if _, err := a.db.Exec("ALTER TABLE chat_users CHANGE deletedat stateat DATETIME(3)"); err != nil {
			return err
		}

		if _, err := a.db.Exec("ALTER TABLE chat_users DROP INDEX users_deletedat"); err != nil {
			return err
		}

		// Add status to formerly soft-deleted users.
		if _, err := a.db.Exec("UPDATE chat_users SET state=? WHERE stateat IS NOT NULL", t.StateDeleted); err != nil {
			return err
		}

		if _, err := a.db.Exec("ALTER TABLE chat_users ADD INDEX users_state(state)"); err != nil {
			return err
		}
		...
		// Subscriptions
		if _, err := a.db.Exec("ALTER TABLE chat_subscriptions ADD INDEX topics_deletedat(deletedat)"); err != nil {
			return err
		}
...
func (a *adapter) UserCreate(user *t.User) error {
	...
	if _, err = tx.Exec("INSERT INTO chat_users(id,createdat,updatedat,state,access,public,tags) VALUES(?,?,?,?,?,?,?)",
		decoded_uid,
...
func (a *adapter) AuthGetUniqueRecord(unique string) (t.Uid, auth.Level, []byte, time.Time, error) {
	...
	if err := a.db.Get(&record, "SELECT userid,secret,expires,authlvl FROM chat_auth WHERE uname=?", unique); err != nil {
		if err == sql.ErrNoRows {
			// Nothing found - clear the error
			err = nil
		}
...
type AntUser struct {
	ID            uint64    `json:"id" gorm:"primary_key" json:"id"`
	Avatar        string    `json:"avatar" gorm:"type:varchar(256)" json:"avatar"`
	Nickname      string    `json:"nickname" gorm:"type:varchar(50);not null;column:nickname"`
	RecentLoginAt time.Time `json:"last_login_time"`
...
func (a *adapter) UserGet(uid t.Uid) (*t.User, error) {
	...
	err := a.userDB.Get(&antUser, "SELECT id,avatar,nickname FROM ant_users WHERE id=?", store.DecodeUid(uid))
	if err == nil {
		user.SetUid(uid)
		user.State = t.StateOK
		user.CreatedAt = antUser.CreatedAt
		user.UpdatedAt = antUser.UpdatedAt
		user.LastSeen = &antUser.RecentLoginAt
		user.Public = map[string]interface{}{"fn": antUser.Nickname, "photo": map[string]string{"data": antUser.Avatar, "type": "jpg"}}
		return &user, nil
	}
...
func (a *adapter) UserGetAll(ids ...t.Uid) ([]t.User, error) {
	...
	users := []t.User{}
	q, uids, _ := sqlx.In("SELECT * FROM ant_users WHERE id IN (?) AND state!=?", uids, t.StateDeleted)
	q = a.db.Rebind(q)
...
func (a *adapter) UserDelete(uid t.Uid, hard bool) error {
	...
	if hard {
		...
		if _, err = tx.Exec("DELETE FROM chat_dellog WHERE deletedfor=?", decoded_uid); err != nil {
			return err
		}
		...
		if _, err = tx.Exec("DELETE chat_sub FROM chat_subscriptions AS sub LEFT JOIN chat_topics ON chat_topics.name=sub.topic WHERE chat_topics.owner=?",
			decoded_uid); err != nil {
			return err
		}

		// Delete topic tags
		if _, err = tx.Exec("DELETE chat_topictags FROM chat_topictags LEFT JOIN chat_topics ON chat_topics.name=chat_topictags.topic WHERE chat_topics.owner=?",
			decoded_uid); err != nil {
			return err
		}

		// And finally delete the topics.
		if _, err = tx.Exec("DELETE FROM chat_topics WHERE owner=?", decoded_uid); err != nil {
			return err
		}

		// Delete user's authentication records.
		if _, err = tx.Exec("DELETE FROM chat_auth WHERE userid=?", decoded_uid); err != nil {
			return err
		}
		...
		if _, err = tx.Exec("DELETE FROM chat_usertags WHERE userid=?", decoded_uid); err != nil {
			return err
		}

		if _, err = tx.Exec("DELETE FROM chat_users WHERE id=?", decoded_uid); err != nil {
			return err
		}
	} else {
		now := t.TimeNow()
		...
		if _, err = tx.Exec("UPDATE chat_subscriptions LEFT JOIN chat_topics ON chat_subscriptions.topic=chat_topics.name "+
			"SET chat_subscriptions.updatedat=?, chat_subscriptions.deletedat=? WHERE chat_topics.owner=?",
			now, now, decoded_uid); err != nil {
			return err
		}
		// Disable group topics where the user is the owner.
		if _, err = tx.Exec("UPDATE chat_topics SET updatedat=?, state=?, stateat=? WHERE owner=?",
			now, t.StateDeleted, now, decoded_uid); err != nil {
			return err
		}
		// Disable p2p topics with the user (p2p topic's owner is 0).
		if _, err = tx.Exec("UPDATE chat_topics LEFT JOIN chat_subscriptions ON chat_topics.name=chat_subscriptions.topic "+
			"SET chat_topics.updatedat=?, chat_topics.state=?, chat_topics.stateat=? WHERE chat_topics.owner=0 AND chat_subscriptions.userid=?",
			now, t.StateDeleted, now, decoded_uid); err != nil {
			return err
		}

		// Disable the other user's subscription to a disabled p2p topic.
		if _, err = tx.Exec("UPDATE chat_subscriptions AS s_one LEFT JOIN chat_subscriptions AS s_two "+
		...
		// Disable user.
		if _, err = tx.Exec("UPDATE chat_users SET updatedat=?, state=?, stateat=? WHERE id=?",
			now, t.StateDeleted, now, decoded_uid); err != nil {
			return err
		}
	}
...
func (a *adapter) topicStateForUser(tx *sqlx.Tx, decoded_uid int64, now time.Time, update interface{}) error {
	...
	if _, err = tx.Exec("UPDATE chat_topics SET state=?, stateat=? WHERE owner=? AND state!=?",
	...
	if _, err = tx.Exec("UPDATE chat_topics LEFT JOIN chat_subscriptions ON chat_topics.name=chat_subscriptions.topic "+
		"SET chat_topics.state=?, chat_topics.stateat=? WHERE chat_topics.owner=0 AND chat_subscriptions.userid=? AND chat_topics.state!=?",
		state, now, decoded_uid, t.StateDeleted); err != nil {
		return err
	}
...
func (a *adapter) UserUpdate(uid t.Uid, update map[string]interface{}) error {
	...
	_, err = tx.Exec("UPDATE ant_users SET "+strings.Join(cols, ",")+" WHERE id=?", args...)
	if err != nil {
		return err
	}
	...
	if tags := extractTags(update); tags != nil {
		...
		_, err = tx.Exec("DELETE FROM chat_usertags WHERE userid=?", decoded_uid)
		if err != nil {
			return err
		}
		// Now insert new tags
...
func (a *adapter) UserUpdateTags(uid t.Uid, add, remove, reset []string) ([]string, error) {
	...
	var allTags []string
	err = tx.Select(&allTags, "SELECT chat_tag FROM usertags WHERE userid=?", decoded_uid)
	if err != nil {
		return nil, err
	}

	_, err = tx.Exec("UPDATE chat_users SET tags=? WHERE id=?", t.StringSlice(allTags), decoded_uid)
	if err != nil {
		return nil, err
	}
...
func (a *adapter) UserGetByCred(method, value string) (t.Uid, error) {
	...
	err := a.db.Get(&decoded_uid, "SELECT userid FROM chat_credentials WHERE synthetic=?", method+":"+value)
	if err == nil {
...
func (a *adapter) UserUnreadCount(uid t.Uid) (int, error) {
	var count int
	err := a.db.Get(&count, "SELECT SUM(t.seqid)-SUM(s.readseqid) FROM chat_topics AS t, chat_subscriptions AS s "+
...
func (a *adapter) topicCreate(tx *sqlx.Tx, topic *t.Topic) error {
	_, err := tx.Exec("INSERT INTO chat_topics(createdat,updatedat,touchedat,state,name,owner,access,public,tags) "+
		"VALUES(?,?,?,?,?,?,?,?,?)",
...
func createSubscription(tx *sqlx.Tx, sub *t.Subscription, undelete bool) error {
	...
	_, err := tx.Exec(
		"INSERT INTO chat_subscriptions(createdat,updatedat,deletedat,userid,topic,modeWant,modeGiven,private) "+
			"VALUES(?,?,NULL,?,?,?,?,?)",
		sub.CreatedAt, sub.UpdatedAt, decoded_uid, sub.Topic, sub.ModeWant.String(), sub.ModeGiven.String(), jpriv)

	if err != nil && isDupe(err) {
		if undelete {
			_, err = tx.Exec("UPDATE chat_subscriptions SET createdat=?,updatedat=?,deletedat=NULL,modeGiven=? "+
				"WHERE topic=? AND userid=?",
				sub.CreatedAt, sub.UpdatedAt, sub.ModeGiven.String(), sub.Topic, decoded_uid)

		} else {
			_, err = tx.Exec(
				"UPDATE chat_subscriptions SET createdat=?,updatedat=?,deletedat=NULL,modeWant=?,modeGiven=?,private=? "+
					"WHERE topic=? AND userid=?",
	...
	if err == nil && isOwner {
		_, err = tx.Exec("UPDATE chat_topics SET owner=? WHERE name=?", decoded_uid, sub.Topic)
	}
	return err
}
...
func (a *adapter) TopicsForUser(uid t.Uid, keepDeleted bool, opts *t.QueryOpt) ([]t.Subscription, error) {
	...
	q := `SELECT createdat,updatedat,deletedat,topic,delid,recvseqid,
		readseqid,modewant,modegiven,private FROM chat_subscriptions WHERE userid=?`
...
func (a *adapter) UsersForTopic(topic string, keepDeleted bool, opts *t.QueryOpt) ([]t.Subscription, error) {
	...
	q := `SELECT s.createdat,s.updatedat,s.deletedat,s.userid,s.topic,s.delid,s.recvseqid,
		s.readseqid,s.modewant,s.modegiven,u.avatar,u.nickname,s.private
		FROM chat_subscriptions AS s JOIN ant_users AS u ON s.userid=u.id 
		WHERE s.topic=?`
...
func (a *adapter) OwnTopics(uid t.Uid) ([]string, error) {
	rows, err := a.db.Queryx("SELECT name FROM ant_topics WHERE owner=?", store.DecodeUid(uid))
	if err != nil {
		return nil, err
	}
...
func (a *adapter) TopicDelete(topic string, hard bool) error {
	...
	if hard {
		if _, err = tx.Exec("DELETE FROM chat_subscriptions WHERE topic=?", topic); err != nil {
			return err
		}
		...
		if _, err = tx.Exec("DELETE FROM chat_topictags WHERE topic=?", topic); err != nil {
			return err
		}

		if _, err = tx.Exec("DELETE FROM chat_topics WHERE name=?", topic); err != nil {
			return err
		}
	} else {
		now := t.TimeNow()
		if _, err = tx.Exec("UPDATE chat_subscriptions SET updatedat=?,deletedat=? WHERE topic=?",
			now, now, topic); err != nil {
			return err
		}

		if _, err = tx.Exec("UPDATE chat_topics SET updatedat=?,state=?,stateat=? WHERE name=?",
...
func (a *adapter) TopicUpdateOnMessage(topic string, msg *t.Message) error {
	_, err := a.db.Exec("UPDATE chat_topics SET seqid=?,touchedat=? WHERE name=?", msg.SeqId, msg.CreatedAt, topic)
...
func (a *adapter) TopicUpdate(topic string, update map[string]interface{}) error {
	...
	_, err = tx.Exec("UPDATE chat_topics SET "+strings.Join(cols, ",")+" WHERE name=?", args...)
	if err != nil {
		return err
	}
...
func (a *adapter) TopicOwnerChange(topic string, newOwner t.Uid) error {
	_, err := a.db.Exec("UPDATE chat_topics SET owner=? WHERE name=?", store.DecodeUid(newOwner), topic)
	return err
}
...
func (a *adapter) SubscriptionGet(topic string, user t.Uid) (*t.Subscription, error) {
	...
	err := a.db.Get(&sub, `SELECT createdat,updatedat,deletedat,userid AS user,topic,delid,recvseqid,
		readseqid,modewant,modegiven,private FROM chat_subscriptions WHERE topic=? AND userid=?`,
...
func (a *adapter) SubsForTopic(topic string, keepDeleted bool, opts *t.QueryOpt) ([]t.Subscription, error) {
	q := `SELECT createdat,updatedat,deletedat,userid AS user,topic,delid,recvseqid,
		readseqid,modewant,modegiven,private FROM chat_subscriptions WHERE topic=?`
...
func (a *adapter) SubsUpdate(topic string, user t.Uid, update map[string]interface{}) error {
	...
	cols, args := updateByMap(update)
	q := "UPDATE chat_subscriptions SET " + strings.Join(cols, ",") + " WHERE topic=?"
	args = append(args, topic)
	if !user.IsZero() {
...
func (a *adapter) SubsDelete(topic string, user t.Uid) error {
	now := t.TimeNow()
	res, err := a.db.Exec(
		"UPDATE chat_subscriptions SET updatedat=?,deletedat=? WHERE topic=? AND userid=? AND deletedat IS NULL",
...
func (a *adapter) SubsDelForTopic(topic string, hard bool) error {
	var err error
	if hard {
		_, err = a.db.Exec("DELETE FROM chat_subscriptions WHERE topic=?", topic)
	} else {
		now := t.TimeNow()
		_, err = a.db.Exec("UPDATE chat_subscriptions SET updatedat=?,deletedat=? WHERE topic=? AND deletedat IS NULL",
...
func subsDelForUser(tx *sqlx.Tx, user t.Uid, hard bool) error {
	var err error
	if hard {
		_, err = tx.Exec("DELETE FROM chat_subscriptions WHERE userid=?", store.DecodeUid(user))
	} else {
		now := t.TimeNow()
		_, err = tx.Exec("UPDATE chat_subscriptions SET updatedat=?,deletedat=? WHERE userid=? AND deletedat IS NULL",
...
func (a *adapter) FindUsers(uid t.Uid, req, opt []string) ([]t.Subscription, error) {
	...
	query := "SELECT u.id,u.created_at,u.updated_at,u.nickname,u.avatar,COUNT(*) AS matches " +
		"FROM ant_users AS u LEFT JOIN usertags AS t ON t.userid=u.id " +
...
func (a *adapter) FindTopics(req, opt []string) ([]t.Subscription, error) {
	...
	query := "SELECT t.name AS topic,t.createdat,t.updatedat,t.access,t.public,t.tags,COUNT(*) AS matches " +
		"FROM chat_topics AS t LEFT JOIN chat_topictags AS tt ON t.name=tt.topic " +
...
func (a *adapter) MessageSave(msg *t.Message) error {
	...
	res, err := a.db.Exec(
		"INSERT INTO chat_messages(createdAt,updatedAt,seqid,topic,`from`,head,content) VALUES(?,?,?,?,?,?,?)",
...
func (a *adapter) MessageGetAll(topic string, forUser t.Uid, opts *t.QueryOpt) ([]t.Message, error) {
	...
	rows, err := a.db.Queryx(
		"SELECT m.createdat,m.updatedat,m.deletedat,m.delid,m.seqid,m.topic,m.`from`,m.head,m.content"+
			" FROM chat_messages AS m LEFT JOIN chat_dellog AS d"+
...
func (a *adapter) MessageGetDeleted(topic string, forUser t.Uid, opts *t.QueryOpt) ([]t.DelMessage, error) {
	...
	rows, err := a.db.Queryx("SELECT topic,deletedfor,delid,low,hi FROM chat_dellog WHERE topic=? AND delid BETWEEN ? AND ?"+
...
func messageDeleteList(tx *sqlx.Tx, topic string, toDel *t.DelMessage) error {
	...
	if toDel == nil {
		...
		_, err = tx.Exec("DELETE FROM chat_dellog WHERE topic=?", topic)
		if err == nil {
			_, err = tx.Exec("DELETE FROM chat_messages WHERE topic=?", topic)
		}
		...
		if insert, err = tx.Prepare(
			"INSERT INTO chat_dellog(topic,deletedfor,delid,low,hi) VALUES(?,?,?,?,?)"); err != nil {
			return err
		}
		...
		if err == nil && toDel.DeletedFor == "" {
			...
			_, err = tx.Exec("DELETE fml.* FROM chat_filemsglinks AS fml INNER JOIN chat_messages AS m ON m.id=fml.msgid WHERE "+
				where, args...)
			if err != nil {
				return err
			}

			_, err = tx.Exec("UPDATE chat_messages AS m SET m.deletedAt=?,m.delId=?,m.head=NULL,m.content=NULL WHERE "+
				where,
...
func (a *adapter) MessageAttachments(msgId t.Uid, fids []string) error {
	...
	_, err = a.db.Exec("INSERT INTO chat_filemsglinks(createdat,fileid,msgid) VALUES "+strings.Join(values, ","), args...)
	if err != nil {
		return err
	}

	_, err = tx.Exec("UPDATE chat_fileuploads SET updatedat='"+strNow+"' WHERE id IN (?"+
...
func (a *adapter) DeviceUpsert(uid t.Uid, def *t.DeviceDef) error {
	...
	_, err = tx.Exec("DELETE FROM chat_devices WHERE hash=?", hash)
	if err != nil {
		return err
	}

	// Actually add/update DeviceId for the new user
	_, err = tx.Exec("INSERT INTO chat_devices(userid, hash, deviceId, platform, lastseen, lang) VALUES(?,?,?,?,?,?)",
...
func (a *adapter) DeviceGetAll(uids ...t.Uid) (map[t.Uid][]t.DeviceDef, int, error) {
	...
	q, unums, _ := sqlx.In("SELECT userid,deviceid,platform,lastseen,lang FROM chat_devices WHERE userid IN (?)", unums)
...
func deviceDelete(tx *sqlx.Tx, uid t.Uid, deviceID string) error {
	...
	var res sql.Result
	if deviceID == "" {
		res, err = tx.Exec("DELETE FROM chat_devices WHERE userid=?", store.DecodeUid(uid))
	} else {
		res, err = tx.Exec("DELETE FROM chat_devices WHERE userid=? AND hash=?", store.DecodeUid(uid), deviceHasher(deviceID))
	}
...
func (a *adapter) CredUpsert(cred *t.Credential) (bool, error) {
	...
	if !cred.Done {
		...
		var done bool
		err = tx.Get(&done, "SELECT done FROM chat_credentials WHERE synthetic=?", synth)
		if err == nil {
		...
		_, err = tx.Exec("UPDATE chat_credentials SET deletedat=? WHERE userid=? AND method=? AND done=false",
			now, userId, cred.Method)
		// Assume that the record exists and try to update it: undelete, update timestamp and response value.
		res, err := tx.Exec("UPDATE chat_credentials SET updatedat=?,deletedat=NULL,resp=?,done=0 WHERE synthetic=?",
		...
		_, err = tx.Exec("DELETE FROM chat_credentials WHERE synthetic=?", cred.User+":"+synth)
		if err != nil {
			return false, err
		}
	}
	// Add new record.
	_, err = tx.Exec("INSERT INTO chat_credentials(createdat,updatedat,method,value,synthetic,userid,resp,done) "+
		"VALUES(?,?,?,?,?,?,?,?)",
...
func credDel(tx *sqlx.Tx, uid t.Uid, method, value string) error {
	...
	if method == "" {
		// Case 1
		res, err = tx.Exec("DELETE FROM chat_credentials"+constraints, args...)
		if err == nil {
			if count, _ := res.RowsAffected(); count == 0 {
				err = t.ErrNotFound
			}
	...
	// Case 2.1
	res, err = tx.Exec("DELETE FROM chat_credentials"+constraints+" AND (done=true OR retries=0)", args...)
	if err != nil {
		return err
	}
	if count, _ := res.RowsAffected(); count > 0 {
	...
	res, err = tx.Exec("UPDATE chat_credentials SET deletedat=?"+constraints, args...)
	if err == nil {
		if count, _ := res.RowsAffected(); count >= 0 {
			err = t.ErrNotFound
		}
...
func (a *adapter) CredConfirm(uid t.Uid, method string) error {
	res, err := a.db.Exec(
		"UPDATE chat_credentials SET updatedat=?,done=true,synthetic=CONCAT(method,':',value) "+
...
func (a *adapter) CredFail(uid t.Uid, method string) error {
	_, err := a.db.Exec("UPDATE chat_credentials SET updatedat=?,retries=retries+1 WHERE userid=? AND method=? AND done=false",
		t.TimeNow(), store.DecodeUid(uid), method)
	return err
}
...
func (a *adapter) CredGetActive(uid t.Uid, method string) (*t.Credential, error) {
	...
	err := a.db.Get(&cred, "SELECT createdat,updatedat,method,value,resp,done,retries "+
		"FROM chat_credentials WHERE userid=? AND deletedat IS NULL AND method=? AND done=false",
...
func (a *adapter) CredGetAll(uid t.Uid, method string, validatedOnly bool) ([]t.Credential, error) {
	query := "SELECT createdat,updatedat,method,value,resp,done,retries FROM chat_credentials WHERE userid=? AND deletedat IS NULL"
	args := []interface{}{store.DecodeUid(uid)}
	if method != "" {
		query += " AND method=?"
		args = append(args, method)
...
func (a *adapter) FileStartUpload(fd *t.FileDef) error {
	_, err := a.db.Exec("INSERT INTO chat_fileuploads(id,createdat,updatedat,userid,status,mimetype,size,location)"+
		" VALUES(?,?,?,?,?,?,?,?)",
...
func (a *adapter) FileFinishUpload(fid string, status int, size int64) (*t.FileDef, error) {
	...
	_, err = a.db.Exec("UPDATE chat_fileuploads SET updatedat=?,status=?,size=? WHERE id=?",
...
func (a *adapter) FileGet(fid string) (*t.FileDef, error) {
	...
	err := a.db.Get(&fd, "SELECT id,createdat,updatedat,userid AS user,status,mimetype,size,location "+
		"FROM chat_fileuploads WHERE id=?", store.DecodeUid(id))
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
...
func (a *adapter) FileDeleteUnused(olderThan time.Time, limit int) ([]string, error) {
	...
	query := "SELECT fu.id,fu.location FROM chat_fileuploads AS fu LEFT JOIN chat_filemsglinks AS fml ON fml.fileid=fu.id WHERE fml.id IS NULL "
	var args []interface{}
	if !olderThan.IsZero() {
	...
	if len(ids) > 0 {
		query, ids, _ = sqlx.In("DELETE FROM chat_fileuploads WHERE id IN (?)", ids)
